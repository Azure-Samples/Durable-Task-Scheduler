...
def main() -> None:
    with DurableTaskSchedulerWorker(**get_connection_config()) as worker:
        worker.add_orchestrator(scheduled_agent_orchestration)
        worker.add_activity(run_scheduled_review)
        worker.add_activity(store_report)
        worker.start()
        print("Scheduled agent worker is running. Press Ctrl+C to stop.")
