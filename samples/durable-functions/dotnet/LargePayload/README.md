...
5. Poll the `statusQueryGetUri` from the response until the orchestration completes. The full status payload also includes the original large `input`, so focus on the `runtimeStatus` and `output` fields. The important part looks like this:
