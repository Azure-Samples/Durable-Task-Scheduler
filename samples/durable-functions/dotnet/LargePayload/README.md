...
- This is a **configurable limit, not a hard product limit**. To allow larger payloads, raise `payloadStorageMaxPayloadBytes` in `host.json`. This sample sets it to `15728640` (15 MiB), including JSON serialization overhead.
...
| `PAYLOAD_SIZE_BYTES` | Size of the test payload this sample generates. Keep it below `payloadStorageMaxPayloadBytes` (15 MiB in this sample), leaving room for JSON serialization overhead. | `1572864` |
