...
// Azurite's PUBLIC development-only account, never an Azure account credential.
...
func newHistoryStore(container string) (*exporthistory.AzureBlobHistoryStore, error) {
	options, err := storageOptions(container)
	if err != nil {
		return nil, err
	}
	return exporthistory.NewAzureBlobHistoryStore(options)
}

func storageOptions(container string) (exporthistory.AzureBlobHistoryStoreOptions, error) {
	...
	options := exporthistory.AzureBlobHistoryStoreOptions{ContainerName: container}
