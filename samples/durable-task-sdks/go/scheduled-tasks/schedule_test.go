...
func TestCleanupReturnsDeleteErrors(t *testing.T) {
	deleteErr := errors.New("schedule delete failed")
	fake := &fakeSchedule{deleteError: deleteErr}
	if err := removeSchedule(context.Background(), fake, true); !errors.Is(err, deleteErr) {
		t.Fatalf("delete error was lost: %v", err)
	}
	if fake.deleted {
		t.Fatal("failed deletion was treated as completed")
	}
}
...
func TestCleanupWaitsForAcceptedCreationAndVerifiesDelete(t *testing.T) {
	...
	if err := waitForScheduleDeletion(ctx, fake); err != nil {
		t.Fatal(err)
	}
...
func TestCleanupNeverTreatsAcknowledgementAsAbsence(t *testing.T) {
	...
	defer cancel()
	fake := &fakeSchedule{}
	if err := removeSchedule(ctx, fake, true); err != nil {
		t.Fatal(err)
	}
	if err := waitForScheduleDeletion(ctx, fake); !errors.Is(err, context.DeadlineExceeded) {
...
func TestDemoObservationHonorsCancellation(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	if err := allowReports(ctx, time.Hour); !errors.Is(err, context.Canceled) {
		t.Fatalf("observation did not honor cancellation: %v", err)
	}
	if err := allowReports(context.Background(), 0); err != nil {
		t.Fatal(err)
	}
}
